...
public class WndTradeItem extends Window {
    ...
    public WndTradeItem(final Heap heap) {
        ...
        final int price = sellprice(item);
    ...
    private float createDescription(Item item, boolean forSale) {
        ...
        int sellprice = price(item);
        if (item instanceof Weapon || item instanceof Armor) {
            sellprice = sellprice(item);
        }
        titlebar.label(forSale ?
                Messages.get(this, "sale", item.toString(), /*price( item )*/sellprice) :
    ...
    private int sellprice(Item item) {
        int price = item.price() * 5 * (Dungeon.depth / 5 + 1);
