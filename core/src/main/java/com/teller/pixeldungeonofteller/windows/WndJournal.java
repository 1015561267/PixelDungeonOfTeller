    ...
    private static class CatalogTab extends Component{
        ...
        protected void createChildren() {
            ...
            for (int i = 0; i < NUM_BUTTONS; i++){
                ...
                int t=i;
                if(t>0) t++;
                itemButtons[i].icon(new ItemSprite(ItemSpriteSheet.WEAPON_HOLDER + t, null));
                add( itemButtons[i] );
            }
        ...
        private static class CatalogItem extends ListItem {
            ...
            public CatalogItem(Item item, boolean IDed, boolean seen ) {
                ...
                int t=currentItemIdx;
                if(t>0) t++;

                if (!seen) {
                    icon.copy( new ItemSprite( ItemSpriteSheet.WEAPON_HOLDER + t, null) );
                    label.text("???");
                    label.hardlight( 0x999999 );
                } else if (!IDed) {
                    icon.copy( new ItemSprite( ItemSpriteSheet.WEAPON_HOLDER + t, null) );
