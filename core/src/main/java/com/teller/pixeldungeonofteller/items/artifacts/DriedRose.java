...
import com.teller.pixeldungeonofteller.actors.mobs.Wraith;
...
public class DriedRose extends Artifact {
    ...
    public static class GhostHero extends NPC {
        ...
        protected boolean act() {
            if (Random.Int(10) == 0) damage(new AbsoluteDamage(1, this, this), this);
            if (!isAlive())
                return true;
        ...
        protected Char chooseEnemy() {
            if (enemy == null || !enemy.isAlive() || !Dungeon.level.mobs.contains(enemy) || state == WANDERING) {
                ...
                for (Mob mob : Dungeon.level.mobs) {
                    if (mob.hostile && Dungeon.hero.fieldOfView[mob.pos] && mob.state != mob.PASSIVE) {
                        enemies.add(mob);
                    }
                }
        ...
        public PhysicalDamage damageRoll() {
            int lvl = (HT - 10) / 3;
            return new PhysicalDamage(Random.NormalIntRange(lvl / 2, 5 + lvl), new PhysicalPercentage(0.33f, 0.33f, 0.33f));
