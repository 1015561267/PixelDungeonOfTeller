    ...
    private CellSelector.Listener caster = new CellSelector.Listener() {
        ...
        public void onSelect(Integer target) {
            if (target != null && (Dungeon.level.visited[target] || Dungeon.level.mapped[target])) {
                ...
                } else if (Dungeon.level.solid[chain.path.get(chain.dist)]
                        || (chain.dist > 0 && Dungeon.level.solid[chain.path.get(chain.dist - 1)])
                        || (chain.path.size() > chain.dist + 1 && Dungeon.level.solid[chain.path.get(chain.dist + 1)])
                        //if the player is trying to grapple the edge of the map, let them.
                        || (chain.path.size() == chain.dist + 1)) {
                    int newPos = -1;
                    for (int i : chain.subPath(1, chain.dist)) {
                        if (!Dungeon.level.solid[i] && Actor.findChar(i) == null) newPos = i;
                    }
                    if (newPos == -1) {
                        GLog.w(Messages.get(EtherealChains.class, "does_nothing"));
                    } else {
