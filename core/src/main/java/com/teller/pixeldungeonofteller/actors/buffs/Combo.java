...
public class Combo extends Buff implements ActionIndicator.Action {
    ...
    private CellSelector.Listener finisher = new CellSelector.Listener() {
        ...
        private void doAttack(final Char enemy) {
            ...
            PhysicalDamage damage = (PhysicalDamage) target.damageRoll();

            float dmg = damage.sum();
            ...
            switch (type) {
                ...
                case SLAM:
                    ...
                    PhysicalDamage damageReroll = (PhysicalDamage) target.damageRoll();
                    //int dmgReroll = target.damageRoll();
                    int dmgReroll = (int) damageReroll.sum();
                ...
                case CRUSH:
                    ...
                    for (int i = 1; i < 4; i++) {
                        PhysicalDamage damageReroll2 = (PhysicalDamage) target.damageRoll();
                        int dmgReroll2 = (int) damageReroll2.sum();
                        //dmgReroll = target.damageRoll();
                        if (dmgReroll2 > dmg) dmg = dmgReroll2;
                    }
            ...
            dmg -= enemy.drRoll();
            damage = new PhysicalDamage((int) dmg, new PhysicalPercentage(0.33f, 0.33f, 0.33f));
            damage = (PhysicalDamage) target.attackProc(enemy, damage);
            damage = (PhysicalDamage) enemy.defenseProc(target, damage);
            ...
            switch (type) {
                case CLOBBER:
                    ...
                    break;
                case SLAM:
                    target.SHLD += Math.max((target.MAXSHLD-target.SHLD)/2, dmg / 2);
                    break;
                default:
                    //nothing
                    break;
            ...
            enemy.sprite.bloodBurstA(target.sprite.center(), damage.effictivehpdamage);
            enemy.sprite.flash();
    ...
    public Image getIcon() {
        Image icon;
        if (((Hero) target).belongings.mainhandweapon != null) {
            icon = new ItemSprite(Dungeon.hero.belongings.mainhandweapon.image, null);
        } else {
