...
import com.teller.pixeldungeonofteller.windows.WndRanking;
...
public class RankingsScene extends PixelScene {
    ...
    public void create() {
        ...
        title.setPos((w - title.width()) / 2 , GAP);
        ...
        if (Rankings.INSTANCE.records.size() > 0) {
            ...
            if (Rankings.INSTANCE.totalNumber >= Rankings.TABLE_SIZE) {
                RenderedTextBlock label = PixelScene.renderTextBlock(Messages.get(this, "total") + " ", 8);
                label.hardlight(0xCCCCCC);
                add(label);

                RenderedTextBlock won = PixelScene.renderTextBlock(Integer.toString(Rankings.INSTANCE.wonNumber), 8);
                won.hardlight(Window.SHPX_COLOR);
                add(won);

                RenderedTextBlock total = PixelScene.renderTextBlock("/" + Rankings.INSTANCE.totalNumber, 8);
                total.hardlight(0xCCCCCC);

                total.setPos((w - total.width()) / 2 , top + pos * rowHeight + GAP );
                add(total);
